// js/admin-ordenes.js
// Órdenes de trabajo + seguimiento + inventario automático
...
window.mostrarOrdenesTrabajo = function () {
  ...
  content.innerHTML = `
    <div class="mb-6">
      ...
      <p class="text-sm text-gray-500">Pedidos web · contacto · seguimiento · inventario automático</p>
    </div>
    <div class="flex flex-wrap gap-2 mb-4">
      ...
      <button onclick="filtrarOT('Cancelado')" class="ot-filtro px-3 py-1.5 rounded-xl text-sm bg-white border" data-f="Cancelado">Cancelado</button>
    </div>
...
function renderOrdenesTrabajo() {
  ...
  lista.innerHTML = data.map(p => {
    ...
    const productos = (p.productos || []).map(i =>
      `<div class="flex justify-between text-sm"><span>${i.nombre} × ${i.cantidad}</span><span>Q${(Number(i.precio) * Number(i.cantidad)).toFixed(2)}</span></div>`
    ).join('');

    const invBadge = p.stockDescontado
      ? '<span class="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">Stock descontado</span>'
      : '<span class="text-xs bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full">Stock pendiente</span>';
    ...
    return `
      <div class="bg-white rounded-3xl shadow p-5 border-l-4 ${p.metodo === 'Stripe' ? 'border-green-500' : 'border-yellow-400'}">
        ...
        <div class="grid sm:grid-cols-2 gap-3 mb-3">
          <div>
            ...
            <select id="ot-estado-${p.id}" class="w-full p-2 border rounded-xl text-sm">
              ${['Pendiente','Esperando pago','Pagado','En proceso','En camino','Entregado','Cancelado'].map(e =>
        ...
        <div class="mb-3">
          <label class="text-xs text-gray-500">Nota / actualización</label>
        ...
        <div class="flex flex-wrap gap-2">
          ...
          ${!p.stockDescontado ? `<button onclick="descontarStockPedido('${p.id}')" class="px-4 py-2 bg-orange-500 text-white rounded-xl text-sm font-medium"><i class="fas fa-boxes mr-1"></i> Descontar stock ahora</button>` : ''}
...
const ESTADOS_DESCUENTAN = ['Pagado', 'En proceso', 'En camino', 'Entregado'];

window.descontarStockPedido = async function (id) {
  const p = (window._otData || []).find(x => x.id === id);
  if (!p) return alert('Pedido no encontrado');
  if (p.stockDescontado) return alert('El stock ya fue descontado');

  if (typeof descontarInventario !== 'function') {
    return alert('Módulo de inventario no cargado. Recarga la página.');
  }

  try {
    const res = await descontarInventario(p.productos || [], p.metodo || 'Web', id);
    await db.collection('pedidos').doc(id).update({
      stockDescontado: true,
      stockPendiente: false,
      stockMovimientos: res.movimientos || []
    });
    if (typeof toastAdmin === 'function') {
      toastAdmin('Inventario', 'Stock descontado correctamente', 'ok');
    } else {
      alert('✅ Stock descontado');
    }
  } catch (e) {
...
window.guardarOrdenTrabajo = async function (id) {
  ...
  try {
    ...
    const update = {
      ...
      historial: (data.historial || []).concat([{
        estado: estado,
        fecha: new Date(),
        nota: nota || 'Actualización desde Admin'
      }])
    };

    if (fechaStr) update.fechaEstimada = new Date(fechaStr + 'T12:00:00');

    // === INVENTARIO AUTOMÁTICO ===
    if (ESTADOS_DESCUENTAN.includes(estado) && !data.stockDescontado && typeof descontarInventario === 'function') {
      const res = await descontarInventario(data.productos || [], data.metodo || 'Web', id);
      update.stockDescontado = true;
      update.stockPendiente = false;
      update.stockMovimientos = res.movimientos || [];
    }

    if (estado === 'Cancelado' && data.stockDescontado && typeof devolverInventario === 'function') {
      await devolverInventario(data.productos || [], data.metodo || 'Web', id);
      update.stockDescontado = false;
      update.stockDevuelto = true;
    }
    ...
    if (typeof toastAdmin === 'function') {
      toastAdmin('Listo', 'Estado sincronizado' + (update.stockDescontado ? ' + stock descontado' : ''), 'ok');
    } else {
      alert('✅ Actualizado');
    }
  } catch (e) {
